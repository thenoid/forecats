...
SERVICE_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional("activity_model", default="gemini-2.5-flash-lite"): cv.string,
        vol.Optional("image_model", default="gemini-3-pro-image-preview"): cv.string,
        vol.Optional("max_image_retries", default=4): vol.All(vol.Coerce(int), vol.Range(min=1, max=10)),
        vol.Optional("initial_backoff_seconds", default=2.0): vol.All(vol.Coerce(float), vol.Range(min=0.1, max=120.0)),
        vol.Optional("max_backoff_seconds", default=30.0): vol.All(vol.Coerce(float), vol.Range(min=0.1, max=300.0)),
